...
import {
  ...
  Navigation, AlertCircle, Loader2, X, ArrowRight, Info, History, Trash2
} from "lucide-react";
...
function TrackingContent() {
  ...
  // Auto-trigger search if ?q= param is present
  useEffect(() => {
  ...
  const fetchHistory = async () => {
    ...
    try {
      const res = await fetch(`/api/tracking-history?userId=${encodeURIComponent(user.id)}`);
      if (res.ok) setHistory(await res.json());
    } catch { /* silent */ }
    finally { setHistoryLoading(false); }
  };

  const saveToHistory = async (data: TrackingResult) => {
    if (!user?.id) return;
    try {
  ...
  const deleteFromHistory = async (id: string) => {
    try {
      await fetch(`/api/tracking-history?id=${id}`, { method: "DELETE" });
  ...
  const doSearch = async (containerNumber: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const trackingData = data as TrackingResult;
        setResult(trackingData);
        saveToHistory(trackingData); // persist to DB
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex items-start gap-3 px-4 py-3 rounded-xl border border-blue-100 bg-blue-50">
                  ...
                  <p className="text-xs text-slate-500">
                    ...
                    Fetched in real-time from the carrier network. This search has been saved to your account.
                  </p>
                </div>
              </div>
            </div>
      ...
      {!result && !loading && history.length > 0 && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {history.map(record => {
              const c = statusColors[record.status ?? ""] ?? "#3b82f6";
              return (
                <div key={record.id}
                  className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 hover:border-blue-300 hover:shadow-md transition-all group">
                  <div className="flex items-start justify-between mb-3">
                    <button
                      onClick={() => { setQuery(record.containerNumber); doSearch(record.containerNumber); }}
                      className="font-bold text-slate-900 font-mono text-sm hover:text-blue-600 transition-colors text-left"
                    >
                  ...
                  <div className="space-y-1 text-xs text-slate-400">
                    {record.carrier && <div className="flex items-center gap-1"><Ship className="h-3 w-3" />{record.carrier}</div>}
                    {record.origin && record.destination && (
                      <div className="flex items-center gap-1"><MapPin className="h-3 w-3" />{record.origin} → {record.destination}</div>
                    )}
                    {record.eta && <div className="flex items-center gap-1"><Clock className="h-3 w-3" />ETA: {record.eta}</div>}
                  </div>
                  <div className="mt-3 pt-3 border-t border-slate-50 flex items-center justify-between">
                    <span className="text-[10px] text-slate-300">
                      {new Date(record.searchedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
                    </span>
                    <button
                      onClick={() => { setQuery(record.containerNumber); doSearch(record.containerNumber); }}
